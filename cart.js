    ...
    class CartManager {
        ...
        this.cart.forEach((item, index) => {
            ...
            productField.style.cssText =
            "background: rgba(255,255,255,0.05); padding: 1rem; border-radius: 10px; margin-bottom: 1rem; border: 1px solid rgba(255,255,255,0.1);"
        ...
        if (selectedOption) {
            const shippingCost = Number.parseInt(selectedOption.getAttribute("data-shipping")) || 0
        ...
        const price = Number.parseFloat(item.price.replace(" EGP", ""))
        ...
        const orderData = {
            ...
            transferPhone: formData.get("transferPhone"),
        ...
        if (transferPhone && transferPhone.value) {
            if (!transferPhone.value.startsWith("01")) {
            this.highlightError(transferPhone)
            this.showNotification("Transfer phone number must start with 01", "error")
            isValid = false
            } else {
            this.clearError(transferPhone)
            }
        }
        ...
        if (transferAmount && transferAmount.value) {
            ...
            if (amount < 100) {
            this.highlightError(transferAmount)
            this.showNotification("Transfer amount must be at least 100 EGP", "error")
            isValid = false
            } else {
            this.clearError(transferAmount)
            }
        }
        ...
        const cartItemsText = orderData.items
            .map((item) => `• ${item.name} (Size: ${item.size || "M"}, Qty: ${item.quantity || 1}, Price: ${item.price})`)
            .join("\n")
        ...
        const templateParams = {
            ...
            transfer_phone: orderData.transferPhone,
            transfer_amount: `${orderData.transferAmount} EGP`,
            deposit_info: `Transfer from: ${orderData.transferPhone} | Amount: ${orderData.transferAmount} EGP`,
    ...
    - Transfer Phone: ${orderData.transferPhone}
    - Transfer Amount: ${orderData.transferAmount} EGP
    - Deposit Required: 100 EGP
    - Transfer To: +201015778481 (Vodafone Cash & Instapay)
    ...
    Order Date: ${new Date().toLocaleDateString("en-US", {
        ...
        console.log("Service ID: service_j5hc49k")
        console.log("Template ID: template_4kdiezw")
        ...
        emailjs
            .send("service_j5hc49k", "template_4kdiezw", templateParams)
            .then((response) => {
            console.log("SUCCESS!", response.status, response.text)
            this.showNotification(
                "Order submitted successfully! We received your order and will contact you soon.",
                "success",
            )
        ...
        const bgColor =
            ...
            : "linear-gradient(45deg,rgb(9, 143, 80),rgb(38, 126, 74))"
